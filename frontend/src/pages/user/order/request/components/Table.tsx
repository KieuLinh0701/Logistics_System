...
import {
    canCancelUserShippingRequest,
    canEditUserShippingRequest,
    ...
    const columns: ColumnsType<ShippingRequest> = [
        {
            ...
            render: (_, record) => {
                ...
                const handleNavigate = () => {
                    if (hasPermissionGroup(['GROUP_USER', 'USER_SUPPORT_DETAIL'])) {
                        onDetail(record.id)
                    } else {
                        message.error("Bạn không có quyền xem chi tiết yêu cầu này.");
                    }
                };
            ...
            render: (_, record) => {
                ...
                const handleNavigate = () => {
                    if (hasPermissionGroup(['GROUP_USER', 'USER_ORDER_DETAIL'])) {
                        navigate(`/orders/tracking/${record.orderTrackingNumber}`);
                    } else {
        ...
        },
        {
            ...
            render: (_, record: ShippingRequest) => {
                const canCancel = canCancelUserShippingRequest(record.status) && hasPermissionGroup(['GROUP_USER', 'USER_SUPPORT_CANCEL']);
                const canEdit = canEditUserShippingRequest(record.status) && hasPermissionGroup(['GROUP_USER', 'USER_SUPPORT_EDIT']);
                ...
                return (
                    <Space>
                        {hasPermissionGroup(['GROUP_USER', 'USER_SUPPORT_DETAIL']) && (
                            <Button
