...
export function summarizePlan(routes: AiShipperRoute[]) {
  return routes.reduce(
    (acc, route) => {
      acc.totalOrders += route.stops?.length ?? route.stopCount ?? 0;
...
export function createStopMarkerSvg(color: string, label: string, highlighted: boolean): string {
  const size = highlighted ? 40 : 34;
  const fontSize = label.length > 3 ? 9 : 11;
  const svg = `
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 40 40">
  <circle cx="20" cy="20" r="17" fill="#ffffff" stroke="${color}" stroke-width="${highlighted ? 4 : 3}"/>
  <text x="20" y="24" text-anchor="middle" font-family="Arial,sans-serif" font-size="${fontSize}" font-weight="700" fill="${color}">${label}</text>
</svg>`;
