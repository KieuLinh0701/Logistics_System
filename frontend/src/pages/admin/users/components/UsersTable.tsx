...
const UsersTable: React.FC<UsersTableProps> = ({ data, loading, page, pageSize, total, onPageChange, onEdit, onDelete, roles = [], actionLoadingById = {} }) => {
  const columns: ColumnsType<AdminUser> = useMemo(() => [
    ...
    { title: 'Trạng thái', dataIndex: 'isActive', render: (v: boolean) => v ? 'Hoạt động' : 'Khóa' },
    {
      ...
      render: (_, record) => (
        <UsersActions record={record} onEdit={onEdit} onDelete={onDelete} loading={!!actionLoadingById[record.id]} />
      ),
      width: 120,
    }
