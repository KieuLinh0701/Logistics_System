...
public class ReportRepository {
    ...
    public List<Object[]> reportByOfficeDetailed(LocalDateTime start, LocalDateTime end) {
        String sql = "SELECT o.id as office_id, o.name as office_name, "
            ...
            + "LEFT JOIN (SELECT o.from_office_id as office_id, COUNT(DISTINCT o.id) as failed FROM orders o JOIN order_histories ohf ON ohf.order_id = o.id AND ohf.action = 'FAILED_DELIVERY' WHERE o.created_at BETWEEN :start AND :end GROUP BY o.from_office_id) fd ON fd.office_id = o.id "
    ...
    public List<Object[]> reportByShipperDetailed(LocalDateTime start, LocalDateTime end) {
        String sql = "SELECT u.id as shipper_id, CONCAT(u.last_name, ' ', u.first_name) as shipper_name, u.phone_number, ofc.name as branch_name, "
            ...
            + "LEFT JOIN (SELECT o.shipper_id, COUNT(DISTINCT o.id) as failed FROM orders o JOIN order_histories ohf ON ohf.order_id = o.id AND ohf.action = 'FAILED_DELIVERY' WHERE o.created_at BETWEEN :start AND :end GROUP BY o.shipper_id) fd ON fd.shipper_id = u.id "
    ...
    public Object[] overviewSummary(LocalDateTime start, LocalDateTime end) {
        String sql = "SELECT "
            ...
            + "(SELECT COUNT(DISTINCT o.id) FROM orders o JOIN order_histories ohf ON ohf.order_id = o.id AND ohf.action = 'FAILED_DELIVERY' WHERE o.created_at BETWEEN :start AND :end) as failed, "
